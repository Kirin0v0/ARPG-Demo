...
{
    /// <summary>
    /// 全局时间缩放命令，面向全部角色，仅在目标时间缩放比当前时间缩放小的情况下生效，否则就不生效
    /// </summary>
    public class TimeScaleGlobalCommand : BaseTimeScaleCommand
    {
        private readonly float _targetTimeScale;

        public TimeScaleGlobalCommand(string id, float duration, float targetTimeScale) : base(id, duration)
        {
