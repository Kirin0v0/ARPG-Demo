...
{
    [Serializable]
    public class QuestLevelGoal : BaseQuestGoal
    {
        // 预定义占位符，将占位符名称映射到对应的属性获取方法
        private static readonly
            Dictionary<string, (string placeHolderMeaning, Func<QuestLevelGoal, string> placeHolderOutput)>
        ...
        // 占位符格式化器
        private static readonly QuestPlaceHolderFormatter<QuestLevelGoal> PlaceHolderFormatter =
            new(PlaceHolderDefinitions);

        [Title("目标完成条件")] [SerializeField] private int level;
        ...
        {
            goal.completed = PlayerDataManager.Level >= level;
        }
