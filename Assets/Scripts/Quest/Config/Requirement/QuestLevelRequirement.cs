...
{
    [Serializable]
    public class QuestLevelRequirement: BaseQuestRequirement
    {
        // 预定义占位符，将占位符名称映射到对应的属性获取方法
        private static readonly
            Dictionary<string, (string placeHolderMeaning, Func<QuestLevelRequirement, string> placeHolderOutput)>
            PlaceHolderDefinitions = new(StringComparer.OrdinalIgnoreCase)
            {
                {
                    "targetLevel", ("玩家目标等级", goal => goal.level.ToString())
                },
                { "currentLevel", ("玩家当前等级", goal => goal.PlayerDataManager.Level.ToString()) },
                // 后续可根据需要添加其他属性
            };

        // 占位符格式化器
        private static readonly QuestPlaceHolderFormatter<QuestLevelRequirement> PlaceHolderFormatter =
            new(PlaceHolderDefinitions);

        [Title("需求满足条件")] [SerializeField] private int level;
        ...
        public override void Update(QuestRequirement requirement, float deltaTime)
        {
            ...
            requirement.meet = PlayerDataManager.Level >= level;
        }
